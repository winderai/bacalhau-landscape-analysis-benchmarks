...
**!Important:** Expected output should include: `SecondaryNameNode` and `DataNode`. If `DataNode` is not in the list, run `hadoop datanode` in a separate terminal (impersonating `hadoopuser`).
